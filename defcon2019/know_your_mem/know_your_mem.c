...
static void* put_secret_somewhere_in_memory()
{
    ...
    char *pg = map_page(random_addr()); // mmap page starting from random addess

    strcpy(pg, HEADER); // copy HEADER to memory
    ...
    if (fstat(fd, &st) != 0)
        err(38, "flag fstat");
    ssize_t flag_read = TEMP_FAILURE_RETRY(read(fd, pg + strlen(HEADER), st.st_size)); // read flag onto the mmapped memory
    if (flag_read != st.st_size)
        err(39, "flag read");
    printf("Secret loaded (header + %zd bytes)\n", flag_read);
    close(fd);
    if (mprotect(pg, 4096, PROT_READ) != 0) // read only (?)
        err(40, "mprotect");
...
// Load shellcode symbol from shellcode.so
