...
/* A CPF sock_map is used to store sock objects. This is primarly used
 * for doing socket redirect with CPF helper routines.
 *
 * A sock map may have CPF programs attached to it, currently a program
 ...
 * no CPF programs are attached the sock object may only be used for sock
 * redirect.
 *
 ...
 *
 * For reference this program is similar to devmap used in YDP context
...
struct cpf_stab {
	...
	struct cpf_prog *cpf_parse;
	struct cpf_prog *cpf_verdict;
};
 ...
 */
static inline void cpf_compute_data_end_sk_skb(struct sk_buff *skb)
{
	TCP_SKB_CB(skb)->cpf.data_end = skb->data + skb_headlen(skb);
}
...
{
	struct cpf_prog *prog = READ_ONCE(psock->cpf_verdict);
	int rc;
	...
	skb_orphan(skb);
	/* We need to ensure that CPF metadata for maps is also cleared
	 * when we orphan the skb so that we don't have the possibility
	 * to reference a stale map.
	 */
	TCP_SKB_CB(skb)->cpf.map = NULL;
	skb->sk = psock->sock;
	cpf_compute_data_end_sk_skb(skb);
	preempt_disable();
	rc = (*prog->cpf_func)(skb, prog->insnsi);
	preempt_enable();
	skb->sk = NULL;

	/* Moving return codes from UAPI namespace into internal namespace */
	return rc == SK_PASS ?
		(TCP_SKB_CB(skb)->cpf.map ? __SK_REDIRECT : __SK_PASS) :
		__SK_DROP;
}
...
static void smap_destroy_psock(struct rcu_head *rcu)
{
	 ...
	 * destroy the psock, strparser, and cpf programs. But,
				       ...
				       struct sk_buff *skb)
{
	...
	rcu_read_lock();
	psock = container_of(strp, struct smap_psock, strp);
	prog = READ_ONCE(psock->cpf_parse);
	...
	/* Attach socket for cpf program to use if needed we can do this
...
static void smap_init_progs(struct smap_psock *psock,
			    struct cpf_stab *stab,
			    struct cpf_prog *verdict,
			    struct cpf_prog *parse)
{
	struct cpf_prog *orig_parse, *orig_verdict;

	orig_parse = xchg(&psock->cpf_parse, parse);
	orig_verdict = xchg(&psock->cpf_verdict, verdict);

	if (orig_verdict)
		cpf_prog_put(orig_verdict);
	if (orig_parse)
		cpf_prog_put(orig_parse);
}
...
static void sock_map_remove_complete(struct cpf_stab *stab)
{
	cpf_map_area_free(stab->sock_map);
	kfree(stab);
}
...
static void smap_gc_work(struct work_struct *w)
{
	...
	if (psock->cpf_parse)
		cpf_prog_put(psock->cpf_parse);
	if (psock->cpf_verdict)
		cpf_prog_put(psock->cpf_verdict);
...
static struct cpf_map *sock_map_alloc(union cpf_attr *attr)
{
	struct cpf_stab *stab;
	int err = -EINVAL;
	u64 cost;
	...
	if (attr->max_entries == 0 || attr->key_size != 4 ||
	    attr->value_size != 4 || attr->map_flags & ~CPF_F_NUMA_NODE)
	...
	stab->map.numa_node = cpf_map_attr_numa_node(attr);
	...
	err = cpf_map_precharge_memlock(stab->map.pages);
	if (err)
		goto free_stab;

	err = -ENOMEM;
	stab->sock_map = cpf_map_area_alloc(stab->map.max_entries *
					    sizeof(struct sock *),
					    stab->map.numa_node);
	if (!stab->sock_map)
		goto free_stab;
...
static void sock_map_free(struct cpf_map *map)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
	int i;
...
static int sock_map_get_next_key(struct cpf_map *map, void *key, void *next_key)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
...
struct sock  *__sock_map_lookup_elem(struct cpf_map *map, u32 key)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
...
static int sock_map_delete_elem(struct cpf_map *map, void *key)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
	struct smap_psock *psock;
	int k = *(u32 *)key;
 ...
 *  - CPF verdict/parse programs must use READ_ONCE and xchg operations
 */
static int sock_map_ctx_update_elem(struct cpf_sock_ops_kern *skops,
				    struct cpf_map *map,
				    void *key, u64 flags)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
	struct smap_psock_map_entry *e = NULL;
	struct cpf_prog *verdict, *parse;
	struct sock *osock, *sock;
	struct smap_psock *psock;
	u32 i = *(u32 *)key;
	int err;

	if (unlikely(flags > CPF_EXIST))
		return -EINVAL;
	...
	if (flags == CPF_EXIST && !sock)
		return -ENOENT;
	else if (flags == CPF_NOEXIST && sock)
		return -EEXIST;

	sock = skops->sk;

	/* 1. If sock map has CPF programs those will be inherited by the
	 * sock being added. If the sock is already attached to CPF programs
	 * this results in an error.
	 */
	verdict = READ_ONCE(stab->cpf_verdict);
	parse = READ_ONCE(stab->cpf_parse);

	if (parse && verdict) {
		/* cpf prog refcnt may be zero if a concurrent attach operation
		 * removes the program after the above READ_ONCE() but before
		 * we increment the refcnt. If this is the case abort with an
		 * error.
		 */
		verdict = cpf_prog_inc_not_zero(stab->cpf_verdict);
		if (IS_ERR(verdict))
			return PTR_ERR(verdict);

		parse = cpf_prog_inc_not_zero(stab->cpf_parse);
		if (IS_ERR(parse)) {
			cpf_prog_put(verdict);
	 ...
	 */
	if (psock) {
		if (READ_ONCE(psock->cpf_parse) && parse) {
			err = -EBUSY;
			goto out_progs;
		}
		psock->refcnt++;
	 ...
	 * running. Attach any CPF programs needed.
	 */
...
int sock_map_prog(struct cpf_map *map, struct cpf_prog *prog, u32 type)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
	struct cpf_prog *orig;

	if (unlikely(map->map_type != CPF_MAP_TYPE_SOCKMAP))
		return -EINVAL;

	switch (type) {
	case CPF_SK_SKB_STREAM_PARSER:
		orig = xchg(&stab->cpf_parse, prog);
		break;
	case CPF_SK_SKB_STREAM_VERDICT:
		orig = xchg(&stab->cpf_verdict, prog);
		break;
	default:
		return -EOPNOTSUPP;
	}
...
static void *sock_map_lookup(struct cpf_map *map, void *key)
{
	return NULL;
}

static int sock_map_update_elem(struct cpf_map *map,
				void *key, void *value, u64 flags)
{
	struct cpf_sock_ops_kern skops;
	u32 fd = *(u32 *)value;
...
static void sock_map_release(struct cpf_map *map, struct file *map_file)
{
	struct cpf_stab *stab = container_of(map, struct cpf_stab, map);
	struct cpf_prog *orig;

	orig = xchg(&stab->cpf_parse, NULL);
	if (orig)
		cpf_prog_put(orig);
	orig = xchg(&stab->cpf_verdict, NULL);
	if (orig)
		cpf_prog_put(orig);
}

const struct cpf_map_ops sock_map_ops = {
...
CPF_CALL_4(cpf_sock_map_update, struct cpf_sock_ops_kern *, cpf_sock,
	   struct cpf_map *, map, void *, key, u64, flags)
{
	WARN_ON_ONCE(!rcu_read_lock_held());
	return sock_map_ctx_update_elem(cpf_sock, map, key, flags);
}

const struct cpf_func_proto cpf_sock_map_update_proto = {
	.func		= cpf_sock_map_update,
	.gpl_only	= false,
	.pkt_access	= true,
	.ret_type	= CRET_INTEGER,
	.arg1_type	= CARG_PTR_TO_CTX,
	.arg2_type	= CARG_CONST_MAP_PTR,
	.arg3_type	= CARG_PTR_TO_MAP_KEY,
