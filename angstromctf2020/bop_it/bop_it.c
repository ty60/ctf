...
int main() {
	...
	while ((c = getchar()) != EOF) {
		if (!strcmp(action, actions[3])) { // if action is "Flag it\n"
			char guess[256];
			guess[0] = c;
			...
			if (strncmp(guess, flag, strlen(flag))) {
                // BOF
                // strlen(guess) != guessLen
                // guessLen could be longer because `read` can read NULL bytes
                // and count it, but strlen(guess) will stop couting on a NULL byte.
