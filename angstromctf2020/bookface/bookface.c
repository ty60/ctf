...
void login() {
	...
	if (access(file, F_OK) != -1) {
		...
		if (strcmp(survey, "10\n10\n10\n10\n") != 0) {
            ...
            // If we can write shellcode in 0x0,
            // overwriteing return address to 0x0 should pop a shell.
			*(user->friends) = 0; // BUG
		}
	} else {
...
int main() {
    ...
    // If we carry out the exploit on a certain time,
    // we can know the random() values.
	srand(time(NULL));
